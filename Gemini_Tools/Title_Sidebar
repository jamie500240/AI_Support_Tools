javascript:(function(){
  const CONTAINER_SELECTORS = '[data-message-author-role="assistant"], .markdown, .prose, article';
  const HEADING_SELECTORS = 'h1, h2, h3, h4';

  function cleanupPrevious(){
    let s = window.__titleMinimapState;
    if (!s) return;
    ...
    window.__titleMinimapState = null;
  }
  cleanupPrevious();

  let state = { observer: null, debounce: null, onVisibility: null, onPagehide: null };
  window.__titleMinimapState = state;

  function rebuild(){
    let existing = document.getElementById('heading-minimap');
    if(existing) existing.remove();

    let aiContainers = document.querySelectorAll(CONTAINER_SELECTORS);
    let rawBlocks = [];

    if (aiContainers.length > 0) {
      aiContainers.forEach(container => {
        let headings = Array.from(container.querySelectorAll(HEADING_SELECTORS));
      ...
      rawBlocks = Array.from(document.querySelectorAll(HEADING_SELECTORS));
    }
    ...
    let config = {
      ...
      'h2': { width: '24px', color: '#129e55', hover: '#0b6635', icon: '📍' },
      'h3': { width: '18px', color: '#e37400', hover: '#b05900', icon: '🔹' },
      'h4': { width: '12px', color: '#757575', hover: '#333333', icon: '▪️' }
    };
  ...
  /* 精準過濾：新增/移除節點需命中標題或 AI 容器選擇器，才視為有意義的變動 */
  function mutationLooksRelevant(mutationsList){
    const COMBINED = HEADING_SELECTORS + ', ' + CONTAINER_SELECTORS;
